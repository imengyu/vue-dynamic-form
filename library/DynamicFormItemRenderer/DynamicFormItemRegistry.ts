...
class DynamicFormItemRegistryClass {
  DynamicFormItemRegistryData = new Map<string, DynamicFormItemRegistryItem>();
  ...
  findDynamicFormItemByType(type: string) : DynamicFormItemRegistryItem|null {
    return this.DynamicFormItemRegistryData.get(type) || null;
  }
  /**
   * 配置已注册的表单组件的附加属性
   * @param type 唯一类型名称
  ...
  configItemAdditionalProps(type: string, additionalProps: Record<string, unknown>) {
    const item = this.DynamicFormItemRegistryData.get(type);
    if (!item) {
   ...
   */
  register(type: string, componentInstance: unknown, additionalProps: Record<string, unknown> = {}, valueName = 'value', override = false) : this {
    if (this.DynamicFormItemRegistryData.has(type) && !override) {
      console.warn('[DynamicFormItemRegistry] Type ' + type + ' already exists and cannot be registered twice.');
      return this;
    }
    this.DynamicFormItemRegistryData.set(type, { componentInstance, additionalProps, valueName });
    return this;
  }
  /**
   * 取消注册自定义表单控件
  ...
  unregister(type: string) {
    if (!this.DynamicFormItemRegistryData.has(type)) {
      console.warn('[DynamicFormItemRegistry] Can not unregister nonexistent type ' + type + ' .');
      return this;
    }
    this.DynamicFormItemRegistryData.delete(type);
    return this;
  }
  /**
   * 清空所有已注册的自定义表单控件
   */
  clearAll() {
    this.DynamicFormItemRegistryData.clear();
    return this;
  }
}
 ...
 */
export const DynamicFormItemRegistry = new DynamicFormItemRegistryClass();