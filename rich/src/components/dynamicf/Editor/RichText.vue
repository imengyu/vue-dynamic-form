...
export interface RichTextProps {
   ...
   * @returns 图片地址
   */
  uploadImageCallback?: (file: File) => Promise<string>;
  /**
   * 覆盖初始化选项
   * @param options 
   */
  overrideInitOptions?: (options: RawEditorOptions) => RawEditorOptions;
}
...
const props = withDefaults(defineProps<RichTextProps & {
  ...
  placeholder: '请输入',
  height: 550,
  uploadImageCallback: () => Promise.resolve('')
})
...
const initOptions = computed<RawEditorOptions>(() => {
  ...
  if (props.overrideInitOptions)
    return props.overrideInitOptions(options);
  return options;
});
